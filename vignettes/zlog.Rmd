...
```{r pbc_zlog}
## loop through all laboratory values and lookup their reference limits
pbc[labs] <- lapply(labs, function(l) {
    rl <- lookup_limits(
        age = pbc$age,
        sex = pbc$sex,
        table = reference[reference$param == l,]
    )
    zlog(pbc[, l], limits = rl)
})
```
